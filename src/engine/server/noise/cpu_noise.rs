...
pub struct CPUNoise {
    ...
    world_seed: i32,
    dimension_schema: ThreadlocalDimensionSchema
}

impl CPUNoise {
    pub fn new(world_seed: i32, dimension_schema: ThreadlocalDimensionSchema) -> CPUNoise {
    ...
    -> ([u8; CHUNK_BLOCK_COUNT as usize], [u8; CHUNK_BLOCK_COUNT as usize]) {
        let chunk_seed = get_chunk_seed(self.world_seed, chunk_pos);
        ...
        let (temperature_points, humidity_points) =
            Self::get_biome_points(chunk_seed);
        ...
        self.generate_temperature_and_humidity_map(&chunk_world_pos, sampled_temperature, sampled_humidity)
    }
    ...
    fn generate_temperature_and_humidity_map(&self, chunk_world_pos: &IVec2, sampled_temperature: [(IVec2, f32); BIOME_SAMPLE_POINT_AMOUNT], sampled_humidity: [(IVec2, f32); BIOME_SAMPLE_POINT_AMOUNT])
    -> ([u8; CHUNK_BLOCK_COUNT as usize], [u8; CHUNK_BLOCK_COUNT as usize])
    {
        ...
        let full_world_size_x = (self.dimension_schema.size.x * CHUNK_SIZE as u32) as f32;
        let full_world_size_y = (self.dimension_schema.size.y * CHUNK_SIZE as u32) as f32;

        let (horiz_var, vert_var) = if let Some(adjustments) = &self.dimension_schema.biome_map_adjustments {
